<%-- 
    ...
    Created on : Dec 19, 2017, 9:49:25 AM
    Author     : huong
--%>
...
<div class="row">
    <div class="col-md-4 col-sm-6 col-xs-0">
        ...
        <form action="../user/advanceSearchProcess.jsp" method="POST">
...
%>
                   <input id="cbDisable" type="checkbox" name="isCheck" value="cat">Choose the category(optional)
                   <br>
                   <select id="catDisabled" name="category" class="form-control" disabled>
<%                    
                        for(Category allCategory: allCategories){
%>               
                    
                        <option value="<%=allCategory.getCategoryID()%>"><%=allCategory.getCategoryName()%></option>
...
%>
            ...
            <div class="form-group">
                <input type="checkbox" name="isCheck" value="desc"> Movie Description(optional)
            </div>
...
<script>
    $('#cbDisable').on("click", function(){
        if($(this).is(':checked')){
            $("#catDisabled").removeAttr("disabled");
        }else{
            $("#catDisabled").prop("disabled", true);
        }
